...
myFT.on('instantads', function () {
    ...
    document.getElementById("headline2").innerHTML = splitHeadline(myFT.instantAds.F3_headline_text, "headline2Span", "headline2_span");
...
function setUp() {
    ...
    function frame1_multiline() {
        ...
        //--if F1_headline_text ends with <br> = force F2_headline_text at the bottom and push F1_headline_text upwards, regardless if it fits or not.
        if (last_line.innerHTML == "") {
            ...
            //--if F1_headline_text and F2_headline_text combined fits the container, insert F2_headline_text at the same line.
        } else if (_temp.length <= max_letters_per_line) {
            lettersCanFit = true;
            last_line_clone.outerHTML = "<p style='opacity:0;'>" + last_line.innerHTML + "&nbsp;</p><p id='extra_text'>" + extra_line + "</p>";
            hidden_text_container.innerHTML = extra_line;

            //--if F1_headline_text and F2_headline_text combined doesn't fit the container, push F1_headline_text upwards and insert F2_headline_text at the bottom
        } else {
            lettersCanFit = false;
            last_line_clone.outerHTML = "<p style='opacity:0;'>" + last_line.innerHTML + "</p><br><p id='extra_text'>" + extra_line + "</p>";
            hidden_text_container.innerHTML = extra_line;
        }
        ...
        //    tl.staggerTo(headline1_lines, 0.6, {top:"0", opacity:1, ease:Power2.easeOut, force3D:true}, 0.15);
        tl.staggerTo(headline1_lines, 1, {
            y: -50,
            opacity: 1,
        ...
        if (extra_line != "") {
            if (lettersCanFit) {
                ...
                tl.add(TweenLite.to(last_line, 1, {
                    x: "-=" + (3 + extra_line_width / 2),
                    ease: Power2.easeInOut,
                    force3D: true
                }), "+=0.75");
                tl.add(TweenLite.to(extra_text, 1, {
                    y: -30,
                    opacity: 1,
                    ease: Power2.easeInOut,
                    force3D: true,
                ...
                //        tl.add( TweenLite.to( headline1, 1, {top:"-="+(extra_lisne_height/2), ease:Power2.easeInOut, force3D:true}), "+=0.75" );
                //        tl.add( TweenLite.to( extra_text, 1, {top:0, opacity:1, ease:Power2.easeInOut, force3D:true, onComplete:function(){
                //          setTimeout(frame1_cleanup, 1000);
                //        }}), "-=1.0");

                tl.add(TweenLite.to(headline1, 1, {
                    y: "-=" + (extra_line_height / 2),
                    ease: Power2.easeInOut,
                    force3D: true
                }), "+=0.75");
    ...
    function frame1_cleanup() {
        ...
        tl.staggerTo(headline1_lines, 1.35, {
            opacity: 0,
            y: "+=" + 30,
            ease: Power2.easeInOut,
            force3D: true
        }, -0.15);
        // tl.staggerTo(headline1_lines, 1.35, {top:"+="+30, ease:Power2.easeInOut, force3D:true}, -0.08);
        //    tl.add( TweenLite.to(headline1_lines, 1, {opacity:0, ease:Power2.easeOut, force3D:true}) );
        tl.add(frame2, "-=0.1");
        //    tl.add(frame2, "-=0.5");
        TweenLite.to(extra_text, 1.35, {
            opacity: 0,
            y: "+=" + 30,
            ease: Power2.easeInOut,
            force3D: true
    ...
    function frame2() {
        ...
        tl2.add(
            TweenLite.to(borderDiv, 0.5, {
                borderWidth: "7px 9px 9px 7px",
                ease: Power2.easeOut,
                force3D: true
            }), "-=0.5"
        );
    ...
    // ---------------------------------
    function frame3() {
        tl3.staggerTo(headline2_lines, 0.8, {
            y: -25,
            opacity: 1,
            ease: Power4.easeOut,
            force3D: true
    ...
    function cta_onhover() {
        ...
        if (isCtaAnimating == false) {
            ...
            tl_cta.add(
                TweenLite.to(cta_background_glow, 1.0, {
                    opacity: 1,
                    ease: Power3.easeOut,
                    force3D: true
                }), "-=1.5"
